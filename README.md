...
- Added guardrail of max-Token <250 tokens to keep answers precise and enforced output format to reduce hallucination. After gaurdrails the output for prompt 5 changed a bit vs when guardrails were absent. We did becuase LLM is probabilistic, so we constrained it like a system component. 
...
- Latency: 7.78 seconds (average response latency for a single request from client to server and back to client is 7.78 seconds, which is higher than the acceptable range. This will need optimisation for real-time customer-facing use"
- Prompt tokens: 548
